...
#     QApplication, QMainWindow, QVBoxLayout, QLabel, QPushButton, QComboBox, QFileDialog, QWidget, QHBoxLayout
# )
...
#         # File selection
#         self.file_label = QLabel("Select a .mov file:")
...
#         file_path, _ = QFileDialog.getOpenFileName(self, "Select .mov File", "", "MOV Files (*.mov);;All Files (*)", options=options)
#         if file_path:
...
#         resolution = self.resolution_dropdown.currentText().split(" (")[0]  # Extract resolution
#         output_file = input_file.replace(".mov", "_converted.mp4")

#         # FFmpeg command
#         command = [
#             "ffmpeg", "-i", input_file, "-vf", f"scale={resolution}", "-c:v", "libx264", "-preset", "fast", "-crf", "23", output_file
#         ]

#         try:
#             subprocess.run(command, check=True)
#             self.file_path_label.setText(f"Conversion completed! Saved as {output_file}")
#             self.file_path_label.setStyleSheet("color: green;")
#         except subprocess.CalledProcessError as e:
#             self.file_path_label.setText("Error: Conversion failed.")
